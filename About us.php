<?php
session_start();
if (isset($_SESSION["email"])) {
  // If the user is logged in, display their name
  // Connect to the MySQL database
  $conn = mysqli_connect("localhost", "root", "", "mydb");

  // Get the user's information from the database
  $email = $_SESSION["email"];
  $sql = "SELECT * FROM users WHERE email='$email'";
  $result = mysqli_query($conn, $sql);
  $user = mysqli_fetch_assoc($result);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="About us.css">
    <title>Pirate's Pizza</title>
    <style>
.aaa {
    color:white;
}</style>
</head>
<body background="pictures\pngtree-delicious-pizza-hand-drawn-geometric-black-banner-picture-image_1027884.jpg">
   

            <nav id="navbar">
        <div id="logo">
                <img src="main logo.png" alt="اهلا">
            </div>
        <ul>
        <li><a href="..\index.php">Home</a></li>
            <li><a href="..\Cart.php">Menu</a></li>
            <li><a href="..\offers.php">Offers</a></li>
            <li><a href="..\php\manuy.php">My Cart</a></li></li>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
            <li><lable class="aaa">Welcome <?php echo $user["firstname"]; ?></lable>
            <li><a href="..\logout.php">Logout</a></li>
            <li><img src="..\pictures\hotLine_english.png"></li>
        </ul>
    </nav>
    <br>
    <br>
    <br>
    <br>
    <br>
              <pre>            


    . The Pirate pizza story begins in May 1958. omr , mado ,mohamed and adhem
    . opened a 550-square-foot pizzeria in Wichita , Kansas Dan and Frank, along 
    . with friends Richard Beamerand John Binder, made their own pizzas Roll out
    . the dough with a rolling pin Richard tossed the dough into the air
    . from the pie tin Dan filled the uncut crust with sauce. John flipped
    . the cake out of the tin during the baking process Dan Carney remembers
    . that people were fascinated by the product, watching us throw dough
    . over our heads They were really enjoying themselves.... Carne's
    . created an innovative and elevated dining experience .
    

              </pre>
              <footer id="footer"><br>
                <section class="client-container">
                <div id="clinet">
                    <div class="client-item">
                        <ul>
                            <li><a href="About%20us.html">About Us</a></li>
                            <li><a href="Careers.html">Job Application</a></li>
                            <li><a href="PRIVACY%20POLICY.html">Polices</a></li>
                            <li><a href="QUALITY.html#">Quality</a></li>
                            <li><a href="Customer Service.html">Customer Service</a></li>
                        </ul>
                    <br><br>
                    <pre>Connect With US</pre>
                    <div class="pics">
                        <br>
                   <a href="https://web.whatsapp.com/"target="_blank"><img src="data:image/png;base64,"/></a>
                   <a href="https://www.facebook.com/"target="_blank"><img src="data:image/png;base64,"/></a>
                   <a href="https://twitter.com/"target="_blank"><img src="data:image/png;base64,"/></a>
                   <a href="https://www.youtube.com/"target="_blank"><img src="data:image/png;base64,"/></a>
                </div>
            </div>
            </div>  
            <br>
            <pre class="footp">Copyright &copy; www.PiratePizza.com All Rights Reserved.</pre>
            <br><br>
                </section>
            </footer>
            <script  src="pizza project\js\sign up.js"></script>
</body>
</html>
<?php
} else {
  // If the user is not logged in, display a login button
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="About us.css">
    <title>Pirate's Pizza</title>
</head>
<body background="pictures\pngtree-delicious-pizza-hand-drawn-geometric-black-banner-picture-image_1027884.jpg">
   

            <nav id="navbar">
        <div id="logo">
                <img src="main logo.png" alt="اهلا">
            </div>
        <ul>
        <li><a href="index.php">Home</a></li>
            <li><a href="Cart.php">Menu</a></li>
            <li><a href="offers.php">Offers</a></li>
            <li><a href="php/manuy.php">My Cart</a></li>
            <li><a href="login.php">LOG-IN</a></li>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
            <li><img src="pictures\hotLine_english.png"></li>
        </ul>
    </nav>
    <br>
    <br>
    <br>
    <br>
    <br>
              <pre>            


    . The Pirate pizza story begins in May 1958. omr , mado ,mohamed and adhem
    . opened a 550-square-foot pizzeria in Wichita , Kansas Dan and Frank, along 
    . with friends Richard Beamerand John Binder, made their own pizzas Roll out
    . the dough with a rolling pin Richard tossed the dough into the air
    . from the pie tin Dan filled the uncut crust with sauce. John flipped
    . the cake out of the tin during the baking process Dan Carney remembers
    . that people were fascinated by the product, watching us throw dough
    . over our heads They were really enjoying themselves.... Carne's
    . created an innovative and elevated dining experience .
    

              </pre>
              <footer id="footer"><br>
                <section class="client-container">
                <div id="clinet">
                    <div class="client-item">
                        <ul>
                        <li><a href="About us.php">About Us</a></li>
                    <li><a href="Careers.php">Job Application</a></li>
                    <li><a href="PRIVACY%20POLICY.php">Polices</a></li>
                    <li><a href="QUALITY.php#">Quality</a></li>
                    <li><a href="Customer Service.php">Customer Service</a></li>
                </ul>
                    <br><br>
                    <pre>Connect With US</pre>
                    <div class="pics">
                        <br>
                   <a href="https://web.whatsapp.com/"target="_blank"><img src="data:image/png;base64,"/></a>
                   <a href="https://www.facebook.com/"target="_blank"><img src="data:image/png;base64,"/></a>
                   <a href="https://twitter.com/"target="_blank"><img src="data:image/png;base64,"/></a>
                   <a href="https://www.youtube.com/"target="_blank"><img src="data:image/png;base64,"/></a>
                </div>
            </div>
            </div>  
            <br>
            <pre class="footp">Copyright &copy; www.PiratePizza.com All Rights Reserved.</pre>
            <br><br>
                </section>
            </footer>
            <script  src="pizza project\js\sign up.js"></script>
</body>
</html>

<?php
}
?>
